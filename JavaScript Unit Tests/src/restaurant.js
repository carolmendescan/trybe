...
const createMenu = (menu) => {
  ...
  consumption: [],
  order: (stryng) => {
    if (Object.keys(menu.food).includes(stryng) || Object.keys(menu.drinks).includes(stryng)) {
    objetoRetornado.consumption.push(stryng);
  } else {
  ...
  pay: () => {
    ...
    for (let index = 0; index < objetoRetornado.consumption.length; index += 1) {
      if (Object.keys(menu.food).includes(objetoRetornado.consumption[index])) {
      const item = Object.keys(menu.food).indexOf(objetoRetornado.consumption[index]);
      totalFood += parseFloat(Object.values(menu.food)[item]);
    }
    if (Object.keys(menu.drinks).includes(objetoRetornado.consumption[index])) {
      const item = Object.keys(menu.drinks).indexOf(objetoRetornado.consumption[index]);
      totalDrinks += parseFloat(Object.values(menu.drinks)[item]);
    }
  }
  return parseFloat((totalFood + totalDrinks) * 1.1);
},
};
