...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  ...
  it('Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à área correta para um círculo de raio 3.', () => {
    expect((circle(3)).area).toBe(Math.PI.toFixed(2)*3*3);
  });
  it('Teste se a função retorna, em um objeto, os dados corretos de um círculo de raio 3.', () => {
    expect(circle(3)).toEqual({radius: 3, area: (Math.PI.toFixed(2)* 3 * 3), circumference: (2 *Math.PI.toFixed(2)* 3),});
  });
// O Math.PI.toFixed(2) ele torna o PI que é 3,141592653... para 3,14 porque coloquei para ser apenas dois digitos dentro dos parênteses. 
});
