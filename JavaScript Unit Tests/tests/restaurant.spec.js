...
//   '4: Faça a implementação do item 4 do README no arquivo src/restaurant.js.'
  ...
  it('7: Verifique se, ao chamar uma função associada à chave `order` no objeto retornado, passando uma string como parâmetro - se a string existir nas chaves `food` ou `drink`, deve ser adicionada ao array consumption  - senão, deve exibir a mensagem "Item indisponível" e não adicionar nada ao array  Ex: obj.order `coxinha`) --> [`coxinha`]  Ex: obj.order(`picanha`) --> Exibe "Item indisponível"', () => {
    const meuRestaurante = createMenu({
      food: {'coxinha': 3.90, 'sanduiche': 9.90},
      drinks: {'agua': 3.90, 'cerveja': 6.90}
    });
    ...
    expect(meuRestaurante.consumption).toEqual(expect.arrayContaining(['coxinha']));
  });

//   it('8: Faça a implementação do item 8 do README no arquivo src/restaurant.js.'
  ...
  it('9: Verifique se, ao adicionar três pedidos em sequência, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos.', () => {
    ...
    expect(meuRestaurante.consumption).toContain('agua', 'coxinha', 'sanduiche')
  });
  ...
  it('10: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.', () => {
    ...
    meuRestaurante.order('agua');
    meuRestaurante.order('coxinha');
    expect(meuRestaurante.consumption).toEqual(['sanduiche', 'agua', 'agua', 'coxinha'])
  });
  ...
  it('11: Verifique se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, acrescido de 10%, conforme registrado em `objetoRetornado.consumption`.', () => {
    ...
    meuRestaurante.order('coxinha');
    expect(meuRestaurante.pay()).toBeCloseTo(27.06, 0.01)
  });
