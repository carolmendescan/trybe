...
export default function RecipeInProgress() {
  ...
  const localList = JSON.parse(localStorage.getItem('updatedList')) || [];
  useEffect(() => {
  ...
  const checkboxVerify = ({ target }) => {
    ...
    if (target.checked) {
      ...
      updatedList.splice(updatedList.indexOf(target.value), 1);
    }
              ...
              && (
                ...
                >
                  ...
                  <input
                    value={ e }
                    type="checkbox"
                    defaultChecked={ localList.includes(e) }
                    onChange={ checkboxVerify }
                  />
                </label>)
            );
            return label;
