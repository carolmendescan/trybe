...
export const sumOfProducts = () => {
  const cartPrice = document.querySelectorAll('.cart__products .product__price__value');
  const totalPrice = Array.from(cartPrice)
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async (event) => {
    const infoId = event.target.parentNode.querySelector('.product__id').innerText;
    saveCartID(infoId);
    const cart = document.querySelector('.cart__products');
    const selectedProduct = await fetchProduct(infoId);
    cart.appendChild(createCartProductElement(selectedProduct));
    sumOfProducts();
  });
