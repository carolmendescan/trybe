...
describe('Testa a aplicação StarWars', () => {
 ...
 expect(screen.getByText('Alderaan')).toBeInTheDocument();
  ...
  test('Filtrar os planetas por número "Tatooine"', () => {
    const inputColumn = screen.getByTestId(('column-filter'));
    const inputComparison = screen.getByTestId(('comparison-filter'));
    const inputNumber = screen.getByTestId(('value-filter'));
    userEvent.selectOptions((inputColumn), 'population');
  ...
  test('testa 3 filtros', () => {
    ...
    expect(screen.getByTestId('filter')).toHaveTextContent(/population/i);
    userEvent.click(screen.getByText(/Excluir/i));
    expect(inputColumn).toHaveTextContent(/population/i);

});

test('testa tabela', async () => {
